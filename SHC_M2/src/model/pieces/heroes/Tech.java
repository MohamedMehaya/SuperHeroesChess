...
public class Tech extends ActivatablePowerHero {
	...
	public void moveUp() throws UnallowedMovementException {
		throw new UnallowedMovementException("Direction of movement UP is not allowed for this piece", this,
				Direction.UP);
	}

	@Override
	public void moveDown() throws UnallowedMovementException {
		throw new UnallowedMovementException("Direction of movement DOWN is not allowed for this piece", this,
				Direction.DOWN);
	}

	@Override
	public void moveRight() throws UnallowedMovementException {
		throw new UnallowedMovementException("Direction of movement RIGHT is not allowed for this piece", this,
				Direction.RIGHT);
	}

	@Override
	public void moveLeft() throws UnallowedMovementException {
		throw new UnallowedMovementException("Direction of movement LEFT is not allowed for this piece", this,
				Direction.LEFT);
	}
		...
		if (newPos != null) {
			...
			if (target.getOwner() == getOwner()) {
				...
				if (getGame().getCellAt(newPos.x, newPos.y).getPiece() != null) {
					throw new InvalidPowerTargetException("Tech can not place " + target.getName()
				...
				throw new InvalidPowerTargetException("Tech can not choose " + target.getName()
				...
				if (target instanceof Armored) {
					...
					if (((Armored) target).isArmorUp()) {
						...
						throw new InvalidPowerTargetException(
								"Tech can not hack " + target.getName() + " as target's power is already used", this,
								target);
				}
			...
			if (target instanceof Hero && target.getOwner() == this.getOwner()) {
				...
				if (target instanceof Armored) {
					...
					if (!((Armored) target).isArmorUp()) {
						...
						throw new InvalidPowerTargetException(
								"Tech can not restore " + target.getName() + "'s power as it is already not used", this,
								target);
