...
public class Ranged extends ActivatablePowerHero {
	...
	public void usePower(Direction d, Piece target, Point newPos)
		...
		case UPRIGHT:
		case UPLEFT:
			throw new InvalidPowerDirectionException("Ranged's power cannot be used in direction: " + d, this, d);
		default:
			break;
		}
		...
		if (hit != null) {
			if (hit.getOwner() != getOwner()) {
				...
				JFrame Error1=new JFrame("Invalid");
				Error1.setVisible(true);
				JLabel disp= new JLabel();
				disp.setText("Using Ranged's power in direction " + d.name()
			...
			throw new InvalidPowerDirectionException("Using Ranged's power in direction " + d.name()
					+ " will result in hitting no enemies", this, d);
		}
